...
| Class | Description |
|---|---|
| `Vehicle` | Holds RC number, vehicle type, and required spot type |
| `ParkingSpot` | Individual spot with status (AVAILABLE / OCCUPIED / RESERVED / MAINTENANCE) |
| `Level` | A floor in the parking lot, contains an array of `ParkingSpot`s |
| `Ticket` | Issued on entry; holds vehicle, timestamps, status, and fee amount |
| `Payment` | Records payment method, status, amount, and timestamp |
| `DisplayBoard` | Maps each `SpotType` to its live available count |

### Services

| Service | Responsibility |
|---|---|
| `ParkingLot` | Orchestrates all subsystems; entry point for operations |
| `parkingLotService` | Core logic: spot allocation, ticket management, payment |
| `EntranceGate` | Handles vehicle entry and ticket issuance |
| `ExitGate` | Processes vehicle exit and triggers payment |
| `ParkingAttendant` | Generates parking availability and revenue reports |
| `ParkingRateCalculator` | Calculates fees based on time; enforces daily cap |
| `LRUCacheService` | Generic LRU Cache (bonus data structure implementation) |
...
| Pattern | Where Used |
|---|---|
| **Singleton** | `ParkingLot` instance management |
| **Factory** | Spot and Level creation |
| **Strategy** | Pluggable parking rate calculation logic |
| **Observer** | `DisplayBoard` updates on every entry/exit event |
| **Service Layer** | Clear separation of business logic from controllers |
...
2. Checking available spots on the display board
3. Vehicle exit and fee calculation
4. Generating parking and revenue reports
...
| Table | Purpose |
|---|---|
| `parking_lot` | Lot metadata (name, address, capacity) |
| `level` | Floors within a lot |
| `parking_spot` | Individual spots per floor |
| `vehicle` | Vehicle registry (RC, type) |
| `ticket` | Entry/exit records with timestamps |
| `payment` | Payment records per ticket |
| `parking_attendant` | Staff accounts and access control |
...
1. REST API endpoints for mobile and web integration
2. Database persistence with JPA / Hibernate
3. Real-time spot updates via WebSocket
4. Monthly subscription management
5. Advance reservation system
6. Dedicated handicap spot management
7. Multi-currency payment support
8. Third-party payment gateway integration
9. Vehicle compaction alerts
10. Electric vehicle (EV) charging station support
...
| File | Description |
|---|---|
| [`PARKING_LOT_LLD.md`](./PARKING_LOT_LLD.md) | Detailed LLD design document |
| [`ARCHITECTURE_DIAGRAM.md`](./ARCHITECTURE_DIAGRAM.md) | Class relationships & sequence diagrams |
| [`FILE_INVENTORY.md`](./FILE_INVENTORY.md) | Full file listing with descriptions |
...
**Rohitash Samota**
- GitHub: [@Rohitash-Samota](https://github.com/Rohitash-Samota)
